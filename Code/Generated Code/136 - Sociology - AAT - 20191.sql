-- From R, need I, 20191, Sociology, AAT, 'SOC 1', 'SOC 1 H' -- 'BLANK', 1 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('SOC 1', 'SOC 1 H') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('MATH 10', 'PSYC 10', 'PSYC 10 H', 'ECON 10', 'ECON 10 H', 'MGT 10', 'MGT 10 H', 'SOC 2') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('SOC 10', 'SOC 19', 'SOC 20', 'SOC 30', 'PSYC 30') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('SOC 3', 'SOC 15', 'HD 15') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
