-- From R, need I, 20191, History, AAT, 'HIST 1', 'HIST 2', 'HIST 20', 'HIST 21' -- 'BLANK', 4 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('HIST 1', 'HIST 2', 'HIST 20', 'HIST 21') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('HIST 33', 'HIST 40', 'HIST 41', 'HIST 51') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('HIST 10', 'HIST 11', 'HIST 24', 'HIST 25') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
