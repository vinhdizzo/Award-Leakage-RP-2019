-- From R, need I, 20191, Elementary Teacher Education, AAT, 'BIO 1', 'BIO 1 H', 'BIO 1 L', 'COMM 1', 'COMM 1 H', 'ERTH 20', 'GEOG 3', 'GEOG 3 H', 'HIST 1', 'HIST 1 H', 'HIST 20', 'HD 7', 'PSYC 7', 'HD 10', 'LIT 1', 'MATH 120', 'PHYS 20', 'PS 1', 'PS 1 H', 'WR 1', 'WR 1 H', 'ENG 1 A', 'ENG 1 AH' -- 'BLANK', 13 -- 0
...
(
	...
	, max(case when N_Course_01 >= 13 and N_Course_02 >= 1 and N_Course_03 >= 1 and N_Course_04 >= 2 and N_Course_05 >= 0 and N_Course_06 >= 0 and N_Course_07 >= 0 and N_Course_08 >= 0 and N_Course_09 >= 0 and N_Course_10 >= 0 and (a.Ind_Group_01 + a.Ind_Group_02 + a.Ind_Group_03 + a.Ind_Group_04 + a.Ind_Group_05 + a.Ind_Group_06 + a.Ind_Group_07 + a.Ind_Group_08 + a.Ind_Group_09 + a.Ind_Group_10 + a.Ind_Group_11 + a.Ind_Group_12 >= 0) then 1 else 0 end) as Major_Req_Ind
	, max(13 + 1 + 1 + 2 + 0 + 0 + 0 + 0 + 0 + 0) as Major_Req_N
	, max(case when N_Course_01 >= 13 then 0 else 13 - N_Course_01 end) +
		...
		as Gap_Major_Total
	, max(case when N_Course_01 >= 13 then 0 else 13 - N_Course_01 end) as Gap_Major_Course_01
	...
	(
		select
		cur_stud.*
		, 'Elementary Teacher Education' as Award_Name
		, 'AAT' as Award_Type
		...
		, sum(case when dc.CourseID in ('BIO 1', 'BIO 1 H', 'BIO 1 L', 'COMM 1', 'COMM 1 H', 'ERTH 20', 'GEOG 3', 'GEOG 3 H', 'HIST 1', 'HIST 1 H', 'HIST 20', 'HD 7', 'PSYC 7', 'HD 10', 'LIT 1', 'MATH 120', 'PHYS 20', 'PS 1', 'PS 1 H', 'WR 1', 'WR 1 H', 'ENG 1 A', 'ENG 1 AH') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('WR 2', 'WR 2 H') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('ARTH 20', 'MUS 20', 'MUS 20 H', 'TA 11') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('LIT 7', 'LIT 20', 'LIT 21', 'LIT 22', 'LIT 23') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
