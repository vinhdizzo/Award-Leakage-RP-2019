-- From R, need I, 20191, Spreadsheets, CP, 'CIM 112 .1', 'CIM 112 .2', 'CIM 212 .3' -- 'BLANK', 3 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('CIM 112 .1', 'CIM 112 .2', 'CIM 212 .3') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('ACCT 1 A', 'ACCT 114', 'ACCT 204', 'ACCT 215') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
