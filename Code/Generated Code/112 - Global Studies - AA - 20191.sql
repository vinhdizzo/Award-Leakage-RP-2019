-- From R, need I, 20191, Global Studies, AA, 'GLBL 1', 'GLBL 2', 'PS 14', 'PS 14 H' -- 'BLANK', 3 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('GLBL 1', 'GLBL 2', 'PS 14', 'PS 14 H') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('ANTH 2', 'ANTH 2 H', 'GEOG 3', 'GEOG 3 H', 'ECON 2', 'ECON 2 H') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('ECON 6', 'ECON 13', 'GEOG 2', 'GEOG 20', 'HIST 2', 'PS 12', 'PS 12 H') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('PS 21', 'ECON 167', 'PS 167') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
