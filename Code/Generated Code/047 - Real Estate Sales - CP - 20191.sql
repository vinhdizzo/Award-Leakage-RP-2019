-- From R, need I, 20191, Real Estate Sales, CP, 'RE 170', 'RE 172' -- 'BLANK', 2 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Real Estate Sales' as Award_Name
		, 'CP' as Award_Type
		...
		, sum(case when dc.CourseID in ('RE 170', 'RE 172') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('RE 174 A', 'RE 175', 'RE 176 A', 'RE 190', 'RE 195') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
