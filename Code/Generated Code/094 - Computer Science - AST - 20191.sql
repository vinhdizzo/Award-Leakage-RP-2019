-- From R, need I, 20191, Computer Science, AST, 'CS 6 A', 'MATH 30' -- 'BLANK', 1 -- 0
...
(
	...
	, sum(case when dam.BK_CollegeCode=@CoR and dam.BK_AwardTypeCode='AST' and dam.AwardMajorDesc=a.Award_Name then 1 else 0 end) as Award_Activity
	, max(case when N_Course_01 >= 1 and N_Course_02 >= 4 and N_Course_03 >= 1 and N_Course_04 >= 2 and N_Course_05 >= 0 and N_Course_06 >= 0 and N_Course_07 >= 0 and N_Course_08 >= 0 and N_Course_09 >= 0 and N_Course_10 >= 0 and (a.Ind_Group_01 + a.Ind_Group_02 + a.Ind_Group_03 + a.Ind_Group_04 + a.Ind_Group_05 + a.Ind_Group_06 + a.Ind_Group_07 + a.Ind_Group_08 + a.Ind_Group_09 + a.Ind_Group_10 + a.Ind_Group_11 + a.Ind_Group_12 >= 0) then 1 else 0 end) as Major_Req_Ind
	, max(1 + 4 + 1 + 2 + 0 + 0 + 0 + 0 + 0 + 0) as Major_Req_N
	...
	(
		select
		cur_stud.*
		, 'Computer Science' as Award_Name
		, 'AST' as Award_Type
		...
		, sum(case when dc.CourseID in ('CS 6 A', 'MATH 30') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('CS 38', 'CS 40 A', 'CS 41', 'MATH 3 A', 'MATH 3 AH') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('MATH 3 B', 'MATH 3 BH') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('PHYS 4 A', 'PHYS 4 B') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
