-- From R, need I, 20191, Mind Body Therapy, CP, 'DNCE 95' -- 'BLANK', 1 -- 0
...
(
	...
	, max(case
		-- certificates
		when a.Award_Type in ('CC', 'CO', 'CP', 'CV') and latest_gpa.CumulativeGPAAsOfEOT >= 2.0 and N_Course_01 >= 1 and N_Course_02 >= 1 and N_Course_03 >= 3 and N_Course_04 >= 1 and N_Course_05 >= 1 and N_Course_06 >= 1 and N_Course_07 >= 0 and N_Course_08 >= 0 and N_Course_09 >= 0 and N_Course_10 >= 0 and (a.Ind_Group_01 + a.Ind_Group_02 + a.Ind_Group_03 + a.Ind_Group_04 + a.Ind_Group_05 + a.Ind_Group_06 + a.Ind_Group_07 + a.Ind_Group_08 + a.Ind_Group_09 + a.Ind_Group_10 + a.Ind_Group_11 + a.Ind_Group_12 >= 0) then 1
		-- degrees
	...
	(
		select
		cur_stud.*
		, 'Mind Body Therapy' as Award_Name
		, 'CP' as Award_Type
		...
		, sum(case when dc.CourseID in ('DNCE 95') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('DNCE 168', 'ENTR 200') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('DNCE 45', 'DNCE 46', 'DNCE 47', 'DNCE 48') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('DNCE 50', 'DNCE 86', 'DNCE 87', 'DNCE 88') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
		, sum(case when dc.CourseID in ('DNCE 51', 'DNCE 52', 'DNCE 53', 'DNCE 54') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_05
		, sum(case when dc.CourseID in ('DNCE 93', 'DNCE 94', 'DNCE 96') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_06
