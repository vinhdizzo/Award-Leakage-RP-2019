-- From R, need I, 20191, Geography, AAT, 'GEOG 1', 'GEOG 1 L', 'GEOG 2' -- 'BLANK', 3 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('GEOG 1', 'GEOG 1 L', 'GEOG 2') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('GEOG 3', 'GEOG 3 H', 'GEOG 10', 'GEOG 10 H', 'GEOG 12', 'GEOG 38', 'GEOG 102') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('GEOG 20', 'ANTH 2', 'ANTH 2 H') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
