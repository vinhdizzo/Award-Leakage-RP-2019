-- From R, need I, 20191, Recycling And Resource Management, CV, 'SRM 180', 'SRM 185', 'SRM 80', 'SRM 85', 'SRM 190', 'SRM 195' -- 'BLANK', 4 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Recycling And Resource Management' as Award_Name
		, 'CV' as Award_Type
		...
		, sum(case when dc.CourseID in ('SRM 180', 'SRM 185', 'SRM 80', 'SRM 85', 'SRM 190', 'SRM 195') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('ECON 6', 'GEOG 20') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('SRM 168') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
