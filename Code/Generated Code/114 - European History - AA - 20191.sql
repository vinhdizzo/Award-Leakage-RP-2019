-- From R, need I, 20191, European History, AA, 'HIST 1', 'HIST 2', 'HIST 10', 'HIST 11', 'WR 2' -- 'BLANK', 5 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'European History' as Award_Name
		, 'AA' as Award_Type
		...
		, sum(case when dc.CourseID in ('HIST 1', 'HIST 2', 'HIST 10', 'HIST 11', 'WR 2') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('HUM 2', 'HUM 3', 'HUM 22', 'PHIL 10', 'PHIL 11') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
