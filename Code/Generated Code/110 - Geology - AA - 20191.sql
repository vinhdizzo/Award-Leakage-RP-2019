-- From R, need I, 20191, Geology, AA, 'CHEM 1 A', 'CHEM 1 B' -- 'BLANK', 2 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('CHEM 1 A', 'CHEM 1 B') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('GEOL 1', 'ERTH 20', 'MS 20') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('GEOL 2', 'MATH 3 A', 'MATH 3 AH', 'MATH 3 B', 'MATH 3 BH') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('PHYS 2 A', 'PHYS 2 B', 'PHYS 4 A', 'PHYS 4 B') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
		, sum(case when dc.CourseID in ('GEOL 165', 'GEOL 169', 'GEOL 170', 'GEOL 181', 'GEOL 186') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_05
