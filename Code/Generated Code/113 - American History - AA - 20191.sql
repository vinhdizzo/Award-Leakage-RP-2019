-- From R, need I, 20191, American History, AA, 'HIST 20', 'HIST 21', 'WR 2' -- 'BLANK', 3 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'American History' as Award_Name
		, 'AA' as Award_Type
		...
		, sum(case when dc.CourseID in ('HIST 20', 'HIST 21', 'WR 2') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('HIST 1', 'HIST 2', 'HIST 10', 'HIST 11') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('HIST 24', 'HIST 33', 'HIST 51', 'JRNL 40', 'LIT 22', 'LIT 23', 'PS 1') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
