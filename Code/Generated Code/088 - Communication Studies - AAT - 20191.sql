-- From R, need I, 20191, Communication Studies, AAT, 'COMM 1' -- 'BLANK', 1 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Communication Studies' as Award_Name
		, 'AAT' as Award_Type
		...
		, sum(case when dc.CourseID in ('COMM 1') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('COMM 3', 'COMM 5', 'COMM 100') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('COMM 2', 'COMM 9', 'COMM 30', 'JRNL 40', 'COMM 106 A', 'COMM 106 B', 'COMM 106 C') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
