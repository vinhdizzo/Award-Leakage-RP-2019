-- From R, need I, 20191, Assistant Stage Manager, CP, 'TA 48 C', 'TA 168' -- 'BLANK', 2 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Assistant Stage Manager' as Award_Name
		, 'CP' as Award_Type
		...
		, sum(case when dc.CourseID in ('TA 48 C', 'TA 168') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('TA 1', 'TA 10', 'TA 12') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('TA 40') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
