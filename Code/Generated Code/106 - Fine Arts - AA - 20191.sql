-- From R, need I, 20191, Fine Arts, AA, 'ARTH 4', 'TA 27' -- 'BLANK', 2 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('ARTH 4', 'TA 27') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('MUS 1', 'MUS 3', 'MUS 20') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('TA 20', 'TA 21') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('PHOT 1', 'ARTH 1', 'PHOT 54') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
		, sum(case when dc.CourseID in ('COMM 30') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_05
