-- From R, need I, 20191, Technical Theatre, AA, 'TA 1' -- 'BLANK', 1 -- 0
...
(
	...
	, max(1 + 1 + 2 + 3 + 0 + 0 + 0 + 0 + 0 + 0) as Major_Req_N
	...
	(
		select
		cur_stud.*
		, 'Technical Theatre' as Award_Name
		, 'AA' as Award_Type
		...
		, sum(case when dc.CourseID in ('TA 1') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('TA 20', 'TA 25', 'TA 26') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('TA 15', 'TA 15 A', 'TA 15 B', 'TA 16', 'TA 16 A', 'TA 16 B', 'TA 17', 'TA 17 A', 'TA 17 B', 'TA 18', 'TA 18 A', 'TA 18 B', 'TA 19', 'TA 19 A', 'TA 19 B', 'TA 241 A', 'TA 241 B', 'TA 241 C') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('TA 40', 'TA 40 C', 'TA 41', 'TA 42') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
