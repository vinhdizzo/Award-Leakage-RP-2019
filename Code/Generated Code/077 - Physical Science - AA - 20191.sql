-- From R, need I, 20191, Physical Science, AA, 'CHEM 1 A', 'CHEM 1 B', 'MATH 3 A', 'MATH 3 AH', 'MATH 3 B', 'MATH 3 BH', 'PHYS 4 A', 'PHYS 4 B' -- 'BLANK', 6 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Physical Science' as Award_Name
		, 'AA' as Award_Type
		...
		, sum(case when dc.CourseID in ('CHEM 1 A', 'CHEM 1 B', 'MATH 3 A', 'MATH 3 AH', 'MATH 3 B', 'MATH 3 BH', 'PHYS 4 A', 'PHYS 4 B') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
