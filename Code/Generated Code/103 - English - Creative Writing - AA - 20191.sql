-- From R, need I, 20191, English - Creative Writing, AA, 'WR 2', 'WR 2 H', 'WR 10', 'LIT 1', 'LIT 24' -- 'BLANK', 4 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'English - Creative Writing' as Award_Name
		, 'AA' as Award_Type
		...
		, sum(case when dc.CourseID in ('WR 2', 'WR 2 H', 'WR 10', 'LIT 1', 'LIT 24') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('WR 11', 'WR 13') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('LIT 30', 'LIT 31', 'LIT 32', 'LIT 33') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
		, sum(case when dc.CourseID in ('LIT 40', 'LIT 41', 'LIT 43') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_04
		, sum(case when dc.CourseID in ('LIT 7', 'LIT 45', 'LIT 48', 'LIT 49') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_05
