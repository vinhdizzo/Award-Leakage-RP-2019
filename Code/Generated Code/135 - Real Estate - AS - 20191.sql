-- From R, need I, 20191, Real Estate, AS, 'RE 172', 'RE 174 A', 'RE 175', 'RE 176 A' -- 'BLANK', 4 -- 0
...
(
	...
	, sum(case when dam.BK_CollegeCode=@CoR and dam.BK_AwardTypeCode='AS' and dam.AwardMajorDesc=a.Award_Name then 1 else 0 end) as Award_Activity
	, max(case when N_Course_01 >= 4 and N_Course_02 >= 1 and N_Course_03 >= 4 and N_Course_04 >= 0 and N_Course_05 >= 0 and N_Course_06 >= 0 and N_Course_07 >= 0 and N_Course_08 >= 0 and N_Course_09 >= 0 and N_Course_10 >= 0 and (a.Ind_Group_01 + a.Ind_Group_02 + a.Ind_Group_03 + a.Ind_Group_04 + a.Ind_Group_05 + a.Ind_Group_06 + a.Ind_Group_07 + a.Ind_Group_08 + a.Ind_Group_09 + a.Ind_Group_10 + a.Ind_Group_11 + a.Ind_Group_12 >= 0) then 1 else 0 end) as Major_Req_Ind
	, max(4 + 1 + 4 + 0 + 0 + 0 + 0 + 0 + 0 + 0) as Major_Req_N
	...
	(
		...
		, sum(case when dc.CourseID in ('RE 172', 'RE 174 A', 'RE 175', 'RE 176 A') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('ECON 1', 'ECON 1 H', 'ECON 2', 'ECON 2 H', 'ECON 13', 'ECON 20', 'ECON 20 H', 'ACCT 1 B', 'ACCT 1 BH', 'ACCT 204', 'ACCT 215') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('RE 170', 'RE 190', 'RE 195', 'RE 250', 'MGT 12 A', 'MGT 12 AH') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
