-- From R, need I, 20191, Early Childhood Assistant Teacher, CP, 'HD 7', 'HD 15' -- 'BLANK', 2 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Early Childhood Assistant Teacher' as Award_Name
		, 'CP' as Award_Type
		...
		, sum(case when dc.CourseID in ('HD 7', 'HD 15') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('HD 110', 'HD 115', 'HD 120', 'HD 131', 'HD 145', 'HD 150') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
