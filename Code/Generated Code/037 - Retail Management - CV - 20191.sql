-- From R, need I, 20191, Retail Management, CV, 'CIM 107', 'MGT 104' -- 'BLANK', 2 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Retail Management' as Award_Name
		, 'CV' as Award_Type
		...
		, sum(case when dc.CourseID in ('CIM 107', 'MGT 104') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('ACCT 215', 'MGT 120', 'MGT 135') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('MGT 125', 'MGT 282', 'MGT 288') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
