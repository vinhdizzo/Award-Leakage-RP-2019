-- From R, need I, 20191, Business Administration, AST, 'ACCT 1 A', 'ACCT 1 B', 'ECON 1', 'ECON 2', 'MGT 12 A' -- 'BLANK', 5 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Business Administration' as Award_Name
		, 'AST' as Award_Type
		...
		, sum(case when dc.CourseID in ('ACCT 1 A', 'ACCT 1 B', 'ECON 1', 'ECON 2', 'MGT 12 A') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('MATH 10', 'MATH 11') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('CIM 107', 'CS 1', 'MGT 1', 'MGT 104') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
