-- From R, need I, 20191, Early Childhood Education, AST, 'HD 101', 'HD 110', 'HD 115', 'HD 130', 'HD 150', 'HD 181' -- 'BLANK', 6 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'Early Childhood Education' as Award_Name
		, 'AST' as Award_Type
		...
		, sum(case when dc.CourseID in ('HD 101', 'HD 110', 'HD 115', 'HD 130', 'HD 150', 'HD 181') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('HD 7', 'PSYC 7') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('HD 15', 'SOC 15') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
