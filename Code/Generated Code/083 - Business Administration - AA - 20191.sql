-- From R, need I, 20191, Business Administration, AA, 'ACCT 1 A', 'ACCT 1 B', 'ECON 1', 'ECON 2' -- 'BLANK', 4 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('ACCT 1 A', 'ACCT 1 B', 'ECON 1', 'ECON 2') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('CS 1', 'MGT 12 A', 'MATH 11') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
