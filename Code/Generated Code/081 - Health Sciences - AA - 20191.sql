-- From R, need I, 20191, Health Sciences, AA, 'BIO 11', 'BIO 12', 'BIO 15' -- 'BLANK', 3 -- 0
...
(
	...
	(
		...
		, sum(case when dc.CourseID in ('BIO 11', 'BIO 12', 'BIO 15') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
		, sum(case when dc.CourseID in ('BIO 71', 'BIO 72', 'PSYC 1', 'PSYC 1 H', 'SOC 1', 'SOC 1 H', 'PSYC 10', 'PSYC 10 H', 'MATH 10') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_02
		, sum(case when dc.CourseID in ('BIO 10', 'CHEM 4') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_03
