-- From R, need I, 20191, CCNA (Cisco Certified Network Associate) Routing and Switching, CP, 'CIM 181', 'CIM 182', 'CIM 284', 'CIM 284' -- 'BLANK', 4 -- 0
...
(
	...
	(
		select
		cur_stud.*
		, 'CCNA (Cisco Certified Network Associate) Routing and Switching' as Award_Name
		, 'CP' as Award_Type
		...
		, sum(case when dc.CourseID in ('CIM 181', 'CIM 182', 'CIM 284', 'CIM 284') and (dg.SuccessFlag=1 or (dt.BK_TermCode=@Current_TermCode and fe.DropCount=0)) then 1 else 0 end) as N_Course_01
